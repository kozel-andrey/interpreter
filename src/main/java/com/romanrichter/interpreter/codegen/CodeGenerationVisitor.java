...
class FunctionDesc {
    ...
    public FunctionDesc(int functionIndex, int argumentCount, String name) {
...
public class CodeGenerationVisitor extends Visitor {
     ...
     *
     */
    private HashMap<String, List<FunctionDesc>> classFunctions = new HashMap<>();
    ...
    private ArrayList<PatchLocation> objectInitializations = new ArrayList<>();
   ...
   */
    ...
    private void visitDeclarations(String[] names) {
    ...
    public void visit(SingleDeclaration singleDeclaration) {
        visitDeclarations(new String[]{singleDeclaration.getName()});
    }
    ...
    public void visit(CustomClass customClass) {
        ...
        for (Function f : customClass.getFunctions()) {
            ...
            functions.put(f.getName(), new FunctionDesc(functionStartIndex,
                    f.getParameters().length, f.getName()));
            f.accept(this);
        }
    }
    ...
    public void visit(ObjectInitializer objectInitializer) {
        for (Expression expression : objectInitializer.getArguments()) {
        ...
        // Wir können die Instruktion zum Laden der Adresse hier noch nicht generieren; die
        // Funktion wird evtl. erst später assembliert.
        objectInitializations.add(new PatchLocation(objectInitializer.getClassName(),
                objectInitializer.getArguments().length, patchLocation));

        // Padding for the tail call optimization
        for (int i = 0; i < objectInitializer.getArguments().length; i++)
            add(NOP.encode());

        add(CALL.encode(objectInitializer.getArguments().length));

    }

    @Override
    public void visit(MethodCall call) {
        call.getCall().accept(this);
    }
    ...
    public void visit(Program program) {
        ...
        for (Function f : program.getFunctions()) {
            ...
            functions.put(f.getName(), new FunctionDesc(functionStartIndex, f.getParameters().length, f.getName()));
            locals.clear();
            f.accept(this);
        }
        ...
        addFunctionCall(patchLocations);
        addFunctionCall(objectInitializations);
    }

    private void addFunctionCall(List<PatchLocation> pls) {
        for (PatchLocation oi : pls) {
            FunctionDesc fDesc = functions.get(oi.functionName);
            if (fDesc == null)
                throw new RuntimeException("Unknown function " + oi.functionName);
            if (fDesc.argumentCount != oi.argumentCount)
                throw new RuntimeException("Invalid number of function arguments");
            instructions.set(oi.ldiLocation, LDI.encode(fDesc.functionIndex));
